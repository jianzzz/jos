...
boot_alloc(uint32_t n)
{
	...
	//先让nextfree等于内核装入后的最末尾的位置往高地址4KB对齐的位置，并从这里分配空间，分配
	//完了以后， nextfree 继续往上移，并将前一个4KB对齐的位置返回。
...
/*
	根据线性地址返回二级页表项入口。
	如果二级页表页不存在，且create标识为0，则返回NULL。
	否则使用page_alloc函数分配页表空间。
	我们知道，pages结构体数组是用于映射所有内存空间的，page_free_list指针反向的结构体所映射的内存空间均可用。
	page_alloc函数将从 pages 数组空间中由后往前分配可用空间，结构体映射的物理页地址将作为线性地址的页表，
	将其存储到页目录里。
...
//
// 移除线性地址对应的物理页，清空二级页表项，使tlb无效化
// 如果不同的线性地址映射到同一个二级页表项，它们就映射到同一个物理页。
